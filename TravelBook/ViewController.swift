...
class ViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate {
    ...
    func mapView(_: MKMapView, annotationView _: MKAnnotationView, calloutAccessoryControlTapped _: UIControl)
    {
        if selectedTitle != "" {
            ...
            CLGeocoder().reverseGeocodeLocation(requestLocation) { placemarks, _ in
                // closure
                if let placemark = placemarks {
                    if placemark.count > 0 {
                        ...
                        let launchOption = [MKLaunchOptionsDirectionsModeKey: MKLaunchOptionsDirectionsModeKey]
    ...
    @IBAction func onSaveClicked(_: Any) {
        let appDelegate = UIApplication.shared.delegate as! AppDelegate
        let context = appDelegate.persistentContainer.viewContext
